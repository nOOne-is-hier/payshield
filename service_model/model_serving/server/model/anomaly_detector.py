...
print(classification_report(y_val_true, y_val_pred, target_names=['정상 (Class 0)', '이상 (Class 1)']))
...
plt.scatter(normal_df['pca1'], normal_df['pca2'], c='blue', alpha=0.5, label='정상 거래')

# 실제 이상 거래 시각화
anomaly_df = results_df[results_df['is_anomaly_true'] == 1]
plt.scatter(anomaly_df['pca1'], anomaly_df['pca2'], c='gold', marker='*', s=200, edgecolor='black', label='실제 이상 거래')

# 모델이 탐지한 이상 거래에 테두리 표시
detected_anomalies_df = results_df[results_df['is_anomaly_pred'] == 1]
plt.scatter(detected_anomalies_df['pca1'], detected_anomalies_df['pca2'], 
            facecolors='none', edgecolors='red', s=150, linewidth=2, label='모델이 탐지한 이상 거래')
...
plt.legend()
plt.tight_layout()
plt.savefig('./result/anomaly_detection_pca.png')
plt.show()
